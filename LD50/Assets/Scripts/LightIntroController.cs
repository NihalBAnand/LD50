...
public class LightIntroController : InteractableGeneric
{
    ...
    override public void Interaction(GameObject player)
    {
        if (!activated)
        {
            ...
            Camera.main.transform.position = GameObject.Find("light").transform.position;
            Camera.main.transform.position += new Vector3(0, 0, -10);

            player.GetComponent<PlayerController>().atLight = true;
        }
        else
        {
            activated = false;
